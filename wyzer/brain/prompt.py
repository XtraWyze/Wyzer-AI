...
class SystemPromptBuilder:
    ...
    def build(self, world: WorldStateSnapshot, conversation: ConversationState) -> str:
        ...
        prompt = (
            ...
            "Use browser_stop only when the user explicitly refers to Wyzer's managed or automation "
            "browser, or when 'the browser' clearly refers to recent managed-browser work. If the user "
            "explicitly says personal Chrome, my Chrome, desktop Chrome, or the current Chrome window, "
            "use control_named_window. If a bare request such as 'close Chrome' does not have enough "
            "recent context to identify managed versus personal Chrome, ask which one; do not call either "
            "close tool. browser_close_tab closes only one managed-browser tab. "
