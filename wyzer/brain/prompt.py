...
class SystemPromptBuilder:
    ...
    def build(self, world: WorldStateSnapshot, conversation: ConversationState) -> str:
        ...
        prompt = (
            ...
            "The native tool list is capability-scoped, but capability visibility never determines "
            "user intent. If the needed specialized tools are absent, use capability discovery and "
            "activate exactly one relevant pack, then continue on the next native round. Do not "
            "activate packs speculatively, and never treat activation as action evidence. Never "
            "call an unoffered function or substitute an unrelated visible tool just because the "
            "exact tool is absent. Discover browser for webpage work; files for path search, reads, "
            "folders, or file changes; clipboard for clipboard or selected-text work; perception "
            "for visible screen understanding or clicks; desktop_interaction for focused-app typing "
            "or keys; and diagnostics for Windows health investigation. Capability coordination is "
            "not a computer action and does not require a task plan. "
