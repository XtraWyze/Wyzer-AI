    ...
    def build(
        ...
        *,
        ...
        prompt = (
            ...
            "work; multiple calls alone do not require a plan. A software-development request whose "
            "create/edit/run/test/debug work can all be delegated to the coding agent is one direct "
            "capability action and must not use task_plan_create, regardless of its internal steps. For "
            "other complex work with dependencies spanning capabilities, intermediate artifacts, retries, "
            "recovery, or cross-step verification, use task_plan_create as the only first call. Never mix plan creation with action "
            ...
            "Software-development work that requires creating, changing, running, testing, or "
            "debugging code belongs to the coding-agent capability, not generic file tools. "
            "Activate coding-agent tools when absent, then delegate the requested outcome. Treat "
            "one coding delegation as a direct action even though the coding agent internally "
            "inspects, edits, and tests; do not create a main task plan solely for those internal "
            "coding steps. For an explicitly requested new project path, coding_agent_start may "
            "set create_workspace true for that exact path. Never set it merely to repair a typo "
            "or guess a location. When CODING_AGENT_CONTEXT_JSON identifies the relevant prior "
            "session, use coding_agent_message for continue, retry, improve, make usable, run, "
            "test, or fix requests. Use coding_agent_status for questions about where the work is "
            "or what changed. Use coding_agent_cancel only when the current user request explicitly "
            "asks to stop or cancel coding; 'try again' always means continue, not cancel. "
            "Browser tools handle managed pages, URLs, tabs, and search. Files tools handle local paths. "
            "When the requested outcome is to create, write, append, or edit a text-based file, "
            "including .txt, .bat, and .cmd files that are not software-development tasks, use "
            "activate_file_tools and then "
            "the matching direct file-content tool. Never open Notepad or another editor and type the "
            "contents through the desktop UI unless the user explicitly asks to use that editor. A "
            "request for a text file named without an extension means a .txt file; a batch file uses "
            "exactly one .bat extension. Preserve the requested line breaks in file content; for a "
            "batch file, put separate commands on separate lines. "
            ...
            "browser_stop only when the user says managed/Wyzer browser or the request clearly follows "
            "managed-browser work. For an unqualified request to close Chrome, use control_named_window "
            "immediately; it treats personal Chrome windows and Wyzer's managed browser as candidates, "
            "closes the sole candidate, and reports labeled choices when several exist. Keep all_matches "
            "false unless the user explicitly says all; after multiple choices are reported, ask which "
            "returned title or managed browser to close. Webpages use browser_inspect_page. Media "
